...
// initial content in information windows
...
var nycha_pos;
$.getJSON('data/nycha_pos.json', function(json) { nycha_pos = json; [nycha_pos_splits,nycha_posmax] = nychaPosQuantiles();
});
...
// Pre-load data for the SNAP Impact Scores
var snap_pos;
$.getJSON('data/snap_pos.json', function(json) { snap_pos = json; [snap_pos_splits,snap_posmax] = snapPosQuantiles(); });

// Pre-load data for the General Population Impact Scores
var gen_pos;
$.getJSON('data/gen_pos.json', function(json) { gen_pos = json; [gen_pos_splits,gen_posmax] = genPosQuantiles(); });
...
var tier1;
$.getJSON('data/tier_1.json', function(json) { tier1 = json; });
...
//		info = '<p>ID: ' + layer.feature.properties.blockid10 + '</p><p>Normalized Impact Score: ' + layer.feature.properties[orr_column] + '</p>';
//	} 
//	else if ((pop == "snapimpact") && (score == "censusscore")) {
//		info = '<p>ID: '+ layer.feature.properties.blockid10 + '</p><p>Normalized Impact Score: ' + layer.feature.properties[orr_column] + '</p>';
//	} 
//	else if ((pop == "nychaimpact") && (score == "censusscore")) {
//		info = '<p>Development: ' + layer.feature.properties.blockid10 + '</p><p>Normalized Impact Score:' + +layer.feature.properties[orr_column] + '</p>';
//	}
...
function mouseoverPFunction(e) {
	...
	if ((pop == "baseimpact") && (score == "posscore")) {
		$('#infoWindow').html('<p>Business Name: '+ layer.feature.properties['BUSINESS NAME'] + '</p><p>Normalized Impact Score: '+ layer.feature.properties[orr_column] + '</p>');
	} 
	else if ((pop == "snapimpact") && (score == "posscore")) {
		$('#infoWindow').html('<p>Business Name: ' + layer.feature.properties.store_name + '</p><p>Normalized Impact Score: ' + layer.feature.properties[orr_column] + '</p>');
	} 
	else if ((pop == "nychaimpact") && (score == "posscore")) {
		$('#infoWindow').html('<p>Business Name: '+ layer.feature.properties.business_name_1 + '</p><p>Normalized Impact Score: ' + layer.feature.properties[orr_column] + '</p>');
	}
}
...
function createMap() {
	...
	case (pop == "snapimpact") && (score == "censusscore"):
		...
		geojson = L.geoJson(snap_imp, { style : snap_style, onEachFeature : onEachFeature }).addTo(map1);
...
$("#clearscore").click( function() {
					...
					$('#maplegend').fadeOut('fast', function() { $('#legendDiv').remove(); $('#maplegend').fadeIn('fast'); });
					map1.setView([ 40.739061, -73.952654 ], 11);
					if (searchControl) { map1.removeControl(searchControl); }
				});
